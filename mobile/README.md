...
当前移动端发布标识为 `1.0.9 (10)`，包名仍为 `com.kilostrength.kilo_strength`。安装新版 APK 前，若设备上仍有旧版同包名应用，请先卸载旧版，或确认安装器明确显示“覆盖更新”并完成升级；不要只依赖桌面图标判断是否已打开新包。启动后进入“我的 → 应用版本”，确认看到 `1.0.9 (10)` 以及“新版导航：训练/记录、AI/识别已合并”。动作库封面使用 reference JPG，点击封面可查看自动播放 GIF、概览/教学/记录三标签。

本轮账号/会员/额度改动的安装产物由主代理统一生成；版本为 `1.0.9+10`。
