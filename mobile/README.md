...
当前移动端发布标识为 `1.0.11 (12)`，包名仍为 `com.kilostrength.kilo_strength`。安装新版 APK 前，若设备上仍有旧版同包名应用，请先卸载旧版，或确认安装器明确显示“覆盖更新”并完成升级；不要只依赖桌面图标判断是否已打开新包。启动后进入“我的 → 个性化 → 应用语言”，可以在简体中文和 English 之间切换。动作库完整包含 1,324 个数据集动作，每个动作使用独立 reference JPG 封面，并可查看 GIF、概览、教学和训练记录；同名动作会显示可追溯的变体标识。
