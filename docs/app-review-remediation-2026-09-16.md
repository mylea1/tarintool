...
## 服务器部署与待补配置
- 已通过既有 SSH 配置部署到 `/opt/kilo/releases/app-review-20260916-r3`。基于线上最新版本增量修改，保留现有 Web Agent 功能。
- 线上候选版本 70 项后端测试全部通过；切换后健康检查成功，未认证删除请求返回 401。没有删除任何真实用户账号进行验证。
- 原版本及 SQLite 备份位于 `/opt/kilo/backups/app-review-20260916`。
- 服务器已有 Apple shared secret、Bundle ID 和 Client ID；缺少 Sign in with Apple 的 Team ID、Key ID 和专用私钥。已找到的本地 `.p8` 属于 App Store Connect 上传密钥，不能代替 Apple 登录密钥。
- 因此 Apple 登录账号的删除暂时会安全拒绝，必须配置登录专用密钥后做真机验证；不能将本次代码部署视为已满足全部审核条件。
