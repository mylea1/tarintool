...
export function openDatabase(databasePath) {
  ...
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_world_presences_venue ON world_presences(venue_code, expires_at);
    ...
    CREATE INDEX IF NOT EXISTS idx_world_echoes_venue ON world_echoes(venue_code, day_key, updated_at DESC);
    ...
    CREATE INDEX IF NOT EXISTS idx_world_interactions_target ON world_interactions(target_user_id, created_at DESC);
  `);
  const presenceColumns = db.prepare('PRAGMA table_info(world_presences)').all();
  if (!presenceColumns.some((column) => column.name === 'training_level')) {
    db.exec('ALTER TABLE world_presences ADD COLUMN training_level INTEGER NOT NULL DEFAULT 1');
  }
  const echoColumns = db.prepare('PRAGMA table_info(world_echoes)').all();
  if (!echoColumns.some((column) => column.name === 'training_level')) {
    db.exec('ALTER TABLE world_echoes ADD COLUMN training_level INTEGER NOT NULL DEFAULT 1');
  }
